 ...
 * 
 */
public class OppNedMonster extends AbstractMonster {
	private Direction startDir;
	public OppNedMonster(Level level, SpriteLoader loader, int column, int row) {
	...
	public void tick() {
		...
		for(int i = 0; i < Direction.values().length; i++){
			...
			if(null != place && place.isWalkable() && !place.isLethal()){
				...
				facingDirection = Mover.turnAround(facingDirection);
			}
		}
